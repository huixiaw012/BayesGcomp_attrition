###### This simulation is applied to evaluate of our method when the dataset is under complete-case setting
...
results <- foreach(i = num_chains, .combine = rbind) %dopar% {
  ...
  lin.formula <- as.formula(paste("~", paste0("Z.",  1:k, collapse  = "+") ))
   ...
   mod_fit <- riBMfits(data = dat_with_NA,  # A matrix or data frame possibly containing confounder(s), exposure(s), outcome(s) and mortality indicator(s) in temporal order from left to right.
   ...
   pred_mod <- gcomp_risoftbart(data = dat_with_NA,  # A matrix or data frame possibly containing confounder(s), exposure(s), outcome(s) and mortality indicator(s) in temporal order from left to right.
                                var.type = c(rep("X0", 3), rep("X",2), "Y", rep("X", 5), "Y", rep("X", 5), "Y"), # X0=baseline confounders (use Bayesian bootstrap), X=confounders (for example time-varying), FI=exposure/treatment, Y=outcome
                                linear_formula = lin.formula,
                                fixed.regime = NULL, # Estimate the ACE for treated (Fi=1) and controls (Fi=0).
                                J = 2500, # Size of pseudo data. Increase to e.g. 5000
