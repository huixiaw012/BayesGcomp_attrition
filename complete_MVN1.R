...
invisible(parallel::clusterEvalQ(cl, {
  ...
  library(locfit)
  library(rBeta2009)
  source("/home/h/huixiaw/huixiaw/GcomGSBART_simulation/MAR_countries/nonlinear/gamma309016_MVNBeta/Complete_MVN/sim_data_nonlin.R")
...
results <- foreach(i = num_chains, .combine = rbind) %dopar% {
  ...
  #missing_prob <- expit(-1.5 + 1.2*(-dat$Beta[,2] + rnorm(length(dat$Beta[,2]), 0, 0.4) ) ) # moderate informative attrition
 
  #missing_prob <- expit(-1.2 + 2*(-dat$Beta[,2] + rnorm(length(dat$Beta[,2]), 0, 0.5) ) ) # strong informative attrition; partial dependence
...
write.table(results,
            file =  paste0("/home/h/huixiaw/huixiaw/GcomGSBART_simulation/MAR_countries/nonlinear/gamma309016_MVNBeta/Complete_MVN/outputs/complete_MVN_", n_attr ,"Miss","_Chain",
